...
var widgetConfig = {
  ...
  chatbotHeaderBackgroundColor:
    userConfig.chatbotHeaderBackgroundColor || "#fafafa",
...
const html = `
   ...
   <div> 
    ...
    <div id="intuitum-chatbot" class="position-fixed d-none flex-column "
        style="width: 95%; max-width: 475px; height: 70vh; border-radius: .75rem;">
<div id="intuitum-chatbot-header" class="d-flex align-items-center justify-content-center text-light p-2"
    style="background-color: ${widgetConfig.chatbotHeaderBackgroundColor} !important; height: auto;">
...
function addStringToChatHistoryList(str) {
  chatHistoryList.push(str);
}

function concatenateStrings() {
  let stringsToConcatenate;
  if (chatHistoryList.length < 5) {
    // If there are less than five strings, concatenate all of them
    stringsToConcatenate = chatHistoryList;
  } else {
    // If there are five or more strings, concatenate the last five
    stringsToConcatenate = chatHistoryList.slice(-4);
  }

  // Join the strings into a single string
  let concatenatedString = stringsToConcatenate.join("");
...
async function getBotResponse(userMessage) {
  if (userMessage.trim() !== "") {
    ...
    let concatenatedChatHistory = concatenateStrings();
    ...
    const requestBody = {
      ...
      conversation_history: concatenatedChatHistory,
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        addStringToChatHistoryList(`USER: ${userMessage} `);

        addStringToChatHistoryList(`ASSISTANT: ${result} `);
        ...
        if (
          ...
          result =
            "Our server are currently down for development work. Why not check back later?";
        }

        return result;
      } else {
        let result =
          "Our servers are currently down for development work. Why not check back later?";
...
// Define a function to add a user message
...
// Define a function to add a bot message
function addBotMessage(message) {
  ...
  var botTextElement = document.createElement("span");
  botTextElement.classList.add("intuitum-bot-text");
  botTextElement.textContent = message;
  botMessageElement.appendChild(botTextElement);

  scrollContainer.scrollTop = scrollContainer.scrollHeight;
}

// Add default bot and user messages
addUserMessage("This is a sample user message");
addBotMessage("This is a sample bot message");
